...
for file_name in inpPath.glob("*T1*.nii.gz"):
    for i in range (0,number_of_times):
        ...
        for j in range(1,7):
            #moco = tio.transforms.Ghosting(num_ghosts=10,intensity=0.4*j,axis=0,restore=0)
            ...
            if(calc):
                GT = s["image"][tio.DATA].squeeze(0).permute(2, 0, 1).numpy()
                P = subject["image"][tio.DATA].squeeze(0).permute(2, 0, 1).numpy()
