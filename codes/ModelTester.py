...
device = "cuda:6"
inp_path = "/project/mukhopad/tmp/BlurDetection_tmp/Dataset/ixi_root/T2_mini"
_, dataset = datasetLoader(inp_path)
print('Number of subjects in T2 dataset:', len(dataset))
...
with torch.no_grad():
    ...
    batch_img = ic_sample['image'][tio.DATA]
    batch_img = batch_img.permute(0, 3, 1, 2)
    batch_img = tio.Subject(image=tio.ScalarImage(tensor=batch_img))
